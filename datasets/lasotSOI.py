...
class LaSOT_SOI(object):
    ...
    def __init__(self, root_dir, subset='test', return_meta=False, mode='MaxPooling', th=0.1):
        super(LaSOT_SOI, self).__init__()
        ...
        split_file = os.path.join('/mnt/second/wangyipei/SOI/data/lasot', mode+'th_{}'.format(th), 'lasot_%s_info.json' % subset[0])
        ...
        # self.seq_dirs = [os.path.join(root_dir, n, 'img') for n in self.seq_names]
        # self.anno_files = [os.path.join(os.path.dirname(d), 'groundtruth.txt') for d in self.seq_dirs]

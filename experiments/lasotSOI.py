...
class ExperimentLaSOTSOI(ExperimentBASE):
    ...
    def __init__(self, root_dir, save_dir, subset='test', return_meta=False, th=0.1):
        # assert subset.upper() in ['TRAIN', 'TEST']
        self.dataset = LaSOT_SOI(root_dir, subset, return_meta=return_meta, th=th)  # root_dir = /mnt/second/wangyipei/SOI/lasot/data
    ...
    def report(self, tracker_names):
        ...
        for name in tracker_names:
            ...
            for s, (_, anno) in enumerate(self.dataset):
                seq_name = self.dataset.seq_names[s]
                result_dir = os.path.join(self.result_dir[:self.result_dir.find('SOI')+3], 'tracker_result', name, self.dataset.name,

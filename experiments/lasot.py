...
class ExperimentLaSOT(object):
    ...
    def __init__(self, root_dir, save_dir, subset, return_meta=False, th=0.1):
    ...
    def sequences_select(self, tracker_names, root_dir, save_dir, screen_mode='MaxPooling', th=0.1):
        ...
        info_file = os.path.join(save_dir, 'data', self.dataset.name, screen_mode + 'th_{}'.format(th),
    ...
    def choose_seq_for_soi(self, seq_name, ratio, save_dir, screen_mode, th, alpha=0.7):
        ...
        if screen_mode == 'MaxPooling':
            save_path = os.path.join(save_dir, 'data', self.dataset.name, screen_mode + 'th_{}'.format(th), 'data',
                                     seq_name)
        elif screen_mode == 'MAX_SCORE':
            save_path = os.path.join(save_dir, 'data', self.dataset.name, screen_mode + 'alpha_{}'.format(alpha),
                                     'data', seq_name)
        ...
        if os.path.exists(save_path):
            ...
            pass
            # gt_path = os.path.join(file_path, 'groundtruth.txt')
            # shutil.copytree(gt_path, save_path)  # file_path 不保存图片了，节省内存
            makedir(save_path)
